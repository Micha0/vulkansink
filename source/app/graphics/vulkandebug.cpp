...
const char* Vulkan::VkFormatToString(VkFormat format)
{
	static std::map<VkFormat, std::string> VkFormatToStringMap = {
		...
		{VK_FORMAT_BEGIN_RANGE, "VK_FORMAT_BEGIN_RANGE"},
		{VK_FORMAT_END_RANGE, "VK_FORMAT_END_RANGE"},
		{VK_FORMAT_RANGE_SIZE, "VK_FORMAT_RANGE_SIZE"},
		{VK_FORMAT_MAX_ENUM, "VK_FORMAT_MAX_ENUM"}
	};

	if (VkFormatToStringMap.find(format) != VkFormatToStringMap.end())
	{
		return VkFormatToStringMap.at(format).c_str();
	}
	else
	{
		return VkFormatToStringMap.at(VK_FORMAT_UNDEFINED).c_str();
	}
}
